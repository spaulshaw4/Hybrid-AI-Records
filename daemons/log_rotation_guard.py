...
def prune_archives(log_file: str) -> None:
    prefix = log_file + "."
    archives = sorted(
        path for path in (os.path.join(os.path.dirname(log_file), name) for name in os.listdir(os.path.dirname(log_file)))
        if path.startswith(prefix) and path.endswith(".gz")
    )
