...
CREATE UNIQUE INDEX IF NOT EXISTS token_ledger_idempotency_key_uidx
  ON public.token_ledger (idempotency_key) WHERE idempotency_key IS NOT NULL;

CREATE OR REPLACE FUNCTION public.credit_user_tokens(target_user_id uuid, token_amount integer, reason text DEFAULT NULL::text, acting_admin_id uuid DEFAULT NULL::uuid, idempotency_key text DEFAULT NULL::text)
...
CREATE OR REPLACE FUNCTION public.refund_user_tokens(target_user_id uuid, token_amount integer, reason text DEFAULT NULL::text, reference text DEFAULT NULL::text, acting_admin_id uuid DEFAULT NULL::uuid, idempotency_key text DEFAULT NULL::text)
RETURNS TABLE(user_id uuid, balance integer, already_applied boolean)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DROP FUNCTION IF EXISTS public.credit_user_tokens(uuid, integer, text, uuid);
DROP FUNCTION IF EXISTS public.refund_user_tokens(uuid, integer, text, text, uuid);
