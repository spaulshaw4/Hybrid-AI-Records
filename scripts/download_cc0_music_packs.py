"""Download CC0 Freesound music previews into ``raw\\freesound_cc0_music``.

Run::

    python "D:\\MusicDatasets\\download_cc0_music_packs.py"

Re-running skips files already on disk. Pack sounds are named
``pack_<id>_<sound_id>_<slug>.mp3``; unpackaged sounds are ``fs_<id>_<slug>.mp3``.
...
folder. After the download finishes::
    ...
    D:\\MusicDatasets\\scripts\\run_slicing_campaign.ps1 -Execute -Root "D:\\MusicDatasets\\raw\\freesound_cc0_music"
"""
...
SAVE_DIR = r"D:\MusicDatasets\raw\freesound_cc0_music"
...
USER_AGENT = "HybridAIForge-cc0-pack-downloader/1.0 (research; preview-hq-mp3 only)"
SEARCH_FILTER = 'license:"Creative Commons 0" category:Music'
SEARCH_FIELDS = "id,name,previews,pack,license,type,username,duration,category"
MIN_BYTES = 2048
FILENAME_TAG = ""  # music files: pack_<id>_... or fs_<id>_...
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Download Freesound CC0 Music category preview-hq-mp3 files. "
    ...
    parser.add_argument(
        "--save-dir",
        default=SAVE_DIR,
        help="Destination root (default: D:\\MusicDatasets\\raw\\freesound_cc0_music).",
    )
    return parser
...
def run(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72, flush=True)
    print("Freesound CC0 Music pack downloader", flush=True)
    print(
    ...
    with requests.Session() as session:
        ...
        while next_url and (saved + skipped) < target:
            ...
            if page == 1:
                print(f"Freesound reports count={count} matching CC0 Music sounds.", flush=True)
                if not results:
                    print(
                        "[warn] category:Music returned 0 results; "
                        'retrying filter license:"Creative Commons 0" tag:music',
                        flush=True,
                    )
                    params["filter"] = 'license:"Creative Commons 0" tag:music'
                    next_url = SEARCH_URL
                    next_params = params
                    page = 0
                    continue
    ...
    if not dry_run:
        ...
        print(
            ...
            r'-Root "D:\MusicDatasets\raw\freesound_cc0_music"',
            flush=True,
        )
