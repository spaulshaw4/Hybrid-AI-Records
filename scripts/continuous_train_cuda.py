...
def load_blacklist(blacklist_path: str = BLACKLIST_PATH) -> set[str]:
...
def is_blacklisted(path: str, blacklist: set[str]) -> bool:
    """Match ``track/file.wav`` keys, with basename-only lines as fallback."""
    if not blacklist:
        return False
    norm = path.replace("\\", "/")
    ...
    for i, part in enumerate(parts[:-1]):
        if f"{part}/{base}" in blacklist:
            return True
    return False


def _atomic_torch_save(payload: dict, path: str) -> None:
    """Write ``latest.pt`` via replace so the live engine never reads a half file."""
    tmp = f"{path}.tmp"
...
def locked_corpus_online() -> bool:
    return os.path.isdir(LOCKED_ROOT)


def audio_path_available(path: str) -> bool:
    """True when the slice exists on NVMe staging and/or the locked D: root."""
    return os.path.exists(resolve_audio_path(path))
...
class LazyRAMDataset(Dataset):
    ...
    def __init__(
        ...
        print(
            f"[DATASET] Blacklist loaded ({len(self.blacklist):,} names) "
            f"from {BLACKLIST_PATH}",
            flush=True,
        )
    ...
    def _load_manifest(self) -> list[tuple[str, int]]:
        ...
        for path, label_id in cursor.fetchall():
            if is_blacklisted(path, self.blacklist):
                blacklisted += 1
                continue
            if self.require_existing and not audio_path_available(path):
                skipped += 1
                continue
        ...
        if skipped:
            print(
                f"[DATASET] Skipped {skipped:,} missing paths "
                f"(D: offline or not yet staged)",
                flush=True,
            )
        if blacklisted:
            print(
                f"[DATASET] Blacklisted {blacklisted:,} bleed/mismatch slices",
                flush=True,
            )
        return records
    ...
    def refresh(self) -> int:
        self.blacklist = load_blacklist()
        print(
            f"[DATASET] Reloaded blacklist ({len(self.blacklist):,} names) "
            f"before manifest refresh",
            flush=True,
        )
        self.records = self._load_manifest()
...
def run_training() -> None:
    ...
    elif locked_corpus_online():
        build_manifest()
    else:
        raise SystemExit(
            f"No manifest at {DB_PATH} and locked corpus offline ({LOCKED_ROOT})."
        )

    if not locked_corpus_online():
        print(
            f"[WARN] Locked corpus offline ({LOCKED_ROOT}). "
            f"Training on NVMe staging only ({STAGING_ROOT}).",
            flush=True,
        )

    dataset = LazyRAMDataset(db_path=DB_PATH, split="train", require_existing=True)
    if len(dataset) == 0 and locked_corpus_online():
        print("[INIT] Manifest empty — building once before train", flush=True)
        build_manifest()
        dataset = LazyRAMDataset(
            db_path=DB_PATH, split="train", require_existing=True
        )
    if len(dataset) == 0:
        raise SystemExit(
            f"No usable train audio (manifest={DB_PATH}, "
            f"staging={STAGING_ROOT}, locked_online={locked_corpus_online()})."
        )
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        if epoch > start_epoch or mini_epochs_done:
            if locked_corpus_online():
                build_manifest()
                n = dataset.refresh()
                print(
                    f"[EPOCH {epoch}] Manifest refresh -> {n:,} train slices",
                    flush=True,
                )
            else:
                n = dataset.refresh()
                print(
                    f"[EPOCH {epoch}] D: still offline — "
                    f"refresh from SQLite only -> {n:,} available slices",
                    flush=True,
                )
