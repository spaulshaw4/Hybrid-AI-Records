 ...
 * Same Windows-stable graph as production remux (aresample + amix dropout=2).
 * EBU R128 loudnorm is applied in the Gate 6 finish pass.
 */
export const STATIC_MASTER_FFMPEG_FILTER =
  "[0:a]aresample=44100,aformat=sample_fmts=fltp:channel_layouts=stereo[inst];" +
  "[1:a]aresample=44100,aformat=sample_fmts=fltp:channel_layouts=stereo[v];" +
  "[inst][v]amix=inputs=2:duration=first:dropout_transition=2[out]";
