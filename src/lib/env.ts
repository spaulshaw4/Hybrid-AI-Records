...
const KEY_ALIASES: Record<string, readonly string[]> = {
  ...
  // Hybrid pipeline Replicate token (Demucs / CWALO). Do not alias to lyric Gemini keys.
  REPLICATE_API_TOKEN: ["REPLICATE_API_KEY"],
  REPLICATE_API_KEY: ["REPLICATE_API_TOKEN"],
  // Gemini / Co-Producer only — never fall through to the hybrid Demucs token.
  ENGINE_API_KEY: ["LYRIC_ENGINE_API_KEY"],
  LYRIC_ENGINE_API_KEY: ["ENGINE_API_KEY"],
