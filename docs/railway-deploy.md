...
| File | Purpose |
| --- | --- |
| `Dockerfile` | Bun image: install → `prisma generate` → `bun run build` (`ARG`/`ENV` for `VITE_*`) → start `.output/server/index.mjs` |
...
so they must be set as Railway **Variables** on the service (passed as matching
`ARG` names, then `ENV` for Vite during `bun run build`), not only at runtime. Only
publishable/public values belong there — never bake `STRIPE_SECRET_*` or
service-role keys into `VITE_*`. Optional `VITE_SENTRY_DSN` is declared the same
way when you want browser Sentry in the production bundle.

### Local / CI Docker build

Pass the same names as `--build-arg` (or rely on a `.env` / CI env that your
builder maps to build-args):
...
docker build `
  ...
  --build-arg VITE_PAYMENTS_CLIENT_TOKEN=$env:VITE_PAYMENTS_CLIENT_TOKEN `
  --build-arg VITE_SENTRY_DSN=$env:VITE_SENTRY_DSN `
  -t hybrid-ai-forge .
```
...
docker build \
  ...
  --build-arg VITE_PAYMENTS_CLIENT_TOKEN="$VITE_PAYMENTS_CLIENT_TOKEN" \
  --build-arg VITE_SENTRY_DSN="$VITE_SENTRY_DSN" \
  -t hybrid-ai-forge .
```

Provide either `VITE_SUPABASE_ANON_KEY` or `VITE_SUPABASE_PUBLISHABLE_KEY` (the
app accepts both). Omit `VITE_SENTRY_DSN` if unused. Do not commit real key
